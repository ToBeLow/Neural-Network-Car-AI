...
CORNER_REWARD  = True
CORNER_BONUS   = 5.0  # fitness bonus when a diagonal detects curve entry
DIAG_SHORT     = 60   # reading below this = car was near inner wall
DIAG_FAR       = 120  # reading above this = inner wall just opened up (curve detected)
...
def eval_genomes(genomes, config):
    ...
    prev_radars    = [list(car.radars) for car in cars]  # previous frame radar readings for corner detection
    ...
    for frame in range(MAX_FRAMES):
        ...
        for i, car in enumerate(cars):
            ...
            # Diagonal spike reward: radars[1]=-60° and radars[7]=+60°
            # when a diagonal jumps short→far the inner wall just opened up = curve entry
            if CORNER_REWARD and car.vel > 2.0:
                if prev_radars[i][1] < DIAG_SHORT and car.radars[1] > DIAG_FAR:
                    ge[i].fitness += CORNER_BONUS  # left curve entry detected
                elif prev_radars[i][7] < DIAG_SHORT and car.radars[7] > DIAG_FAR:
                    ge[i].fitness += CORNER_BONUS  # right curve entry detected
                prev_radars[i] = list(car.radars)
...
def run(config_path, label, pop_size, n_gens):
    ...
    config.genome_config.num_inputs = len(RADAR_ANGLES)
    ...
    with open(os.path.join(src, "models", f"{label}_winner.pkl"), "wb") as f:
